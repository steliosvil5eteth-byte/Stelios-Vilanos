...
v1.5 supports two daily-data adapters:
...
The validation suite covers all earlier releases plus v1.5 tests for:
...
v1.5 adds a second independent execution safety layer before any paper order can be accepted.

### Two-key global paper execution gate

Paper execution now requires **both**:

1. `PAPER_EXECUTION_ENABLED=true` in the server environment, and
2. the admin-controlled stored switch to be enabled with `emergencyStop=false`.

The stored switch defaults to blocked. Therefore a new deployment cannot submit even simulated paper orders until an administrator deliberately enables it. Live execution remains unavailable.
...
Example body to enable paper execution after the environment gate has also been enabled:

```json
{"paperExecutionEnabled":true,"emergencyStop":false,"note":"paper validation approved"}
```

Setting `emergencyStop=true` immediately blocks subsequent simulated and IBKR paper submissions.

### Stale market-data guard

Scheduled and manual server scans reject a symbol before strategy evaluation if the newest daily bar is older than `MARKET_DATA_MAX_AGE_DAYS` (default `7`). The rejection is recorded as `STALE_MARKET_DATA` in the scan history.

### IBKR reconciliation watchdog

`GET /api/broker/ibkr/reconcile` compares local open paper trades with the IBKR paper account's current positions and visible open orders. It reports, but does not automatically alter, mismatches such as:

- a local open trade with no corresponding broker position,
- an unexpected broker position not represented locally,
- a broker position with no visible protective order,
- duplicate execution IDs in the fetched trade window.

Critical mismatches return HTTP 409. The watchdog is intentionally report-only so an ambiguous symbol/contract match cannot trigger an automatic trade.

### Request tracing and schema validation

Order, scan, health, safety and reconciliation flows emit an `X-Request-Id` and use stricter server-side validation for symbols and long paper-order geometry (`stop < entry < target`).

### Validation

`npm run validate` now includes `scripts/v15-smoke.mjs`, which verifies:

- default execution block,
- two-key enablement,
- emergency-stop enforcement,
- stale/fresh daily data checks,
- order schema rejection,
- symbol normalization,
- healthy broker reconciliation,
- orphan local trade detection,
- unexpected broker-position detection.
...
## v1.7 release operations

- Tamper-evident SHA-256 audit chain for newly appended audit events, with admin verification.
- PostgreSQL migration ledger with advisory-lock serialized migrations and schema version 4.
- Admin migration endpoint requires maintenance mode and exact confirmation phrase.
- Stable/canary release control with deterministic per-user bucketing and release lock.
- Recovery backup validation is dry-run only; it never restores or mutates user data.
- Release check script and expanded admin preflight combine integrity, migrations, audit-chain, maintenance, release lock and safety state.

New endpoints:
- `GET|POST /api/admin/migrations`
- `GET|PUT /api/admin/release-control`
- `GET /api/admin/audit-verify`
- `POST /api/admin/recovery-check`
- `GET /api/release`

Migration workflow:
1. Enable maintenance mode.
2. Run migrations with `confirm=RUN MIGRATIONS`.
3. Verify integrity and audit chain.
4. Set release lock OFF only when ready for deployment.
5. Run preflight/release check.
6. Deploy canary or stable release.
