...
export default async function handler(req,res){
  ...
  try{const rid=requestId(req,res);await rateLimit(req,{label:'paper-order',limit:30,windowMs:10*60*1000});const account=await requireAccount(req),user=account.username,key=requireIdempotencyKey(req);
    await requirePaperConsent(user);requireVerifiedEmail(account);await requirePaperExecutionAllowed();const order=validateLongPaperOrder(req.body?.order||{});
