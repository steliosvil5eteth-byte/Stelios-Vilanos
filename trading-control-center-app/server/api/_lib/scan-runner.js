...
export async function runServerScan(user,{symbols=null,source='manual',maxSymbols=5}={}){
  ...
  for(const item of fetched){
    ...
    const fresh=marketFreshness(item.series);
    if(!fresh.ok){evaluations.push({symbol:item.symbol,accepted:false,reasons:[`STALE_MARKET_DATA:${fresh.reason}`],bars:item.series.length,freshness:fresh});continue}
    const ev=evaluateSeries(item.symbol,item.series,settings);ev.freshness=fresh;evaluations.push(ev);
  }
