...
export async function fetchAlphaDaily(symbol,{apiKey=process.env.ALPHAVANTAGE_API_KEY,outputsize=process.env.ALPHAVANTAGE_OUTPUTSIZE||'compact'}={}){
  ...
  const r=await fetch(u,{headers:{'User-Agent':'Trading-Control-Center/1.7'}});if(!r.ok)throw new Error(`HTTP ${r.status}`);const json=await r.json();if(json.Note)throw new Error('API rate limit');if(json.Information)throw new Error(json.Information);if(json['Error Message'])throw new Error('Unknown/unsupported symbol');const series=parseDaily(json);if(!series?.length)throw new Error('No daily series returned');return {symbol,series,meta:json['Meta Data']||{},provider:'alphavantage',outputsize:size};
}
export async function fetchTwelveDaily(symbol,{apiKey=process.env.TWELVE_DATA_API_KEY,outputsize=process.env.TWELVE_DATA_OUTPUTSIZE||'1000'}={}){
  ...
  const r=await fetch(u,{headers:{'User-Agent':'Trading-Control-Center/1.7'}});if(!r.ok)throw new Error(`HTTP ${r.status}`);const json=await r.json();if(json.status==='error'||json.code)throw new Error(json.message||`Twelve Data error ${json.code}`);const series=parseTwelveDaily(json);if(!series?.length)throw new Error('No daily series returned');return {symbol,series,meta:json.meta||{},provider:'twelvedata',outputsize:size};
}
