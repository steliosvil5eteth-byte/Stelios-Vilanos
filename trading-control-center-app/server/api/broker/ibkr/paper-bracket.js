...
export default async function handler(req,res){
  ...
  try{
    ...
    const rid=requestId(req,res);const account=await requireAccount(req);requireFeature(account,'brokerPaper');const user=account.username,key=requireIdempotencyKey(req);
    await requirePaperConsent(user);requireVerifiedEmail(account);await requirePaperExecutionAllowed();const order=validateBracketOrder(req.body||{});
