...
export default async function handler(req,res){
 try{const account=await requireAccount(req),user=account.username,key=`tcc:${user}:alerts`;
  if(req.method==='GET'){const a=(await getJson(key))||{rules:{nearStopPct:1,nearTargetPct:1,dailyLossWarnPct:80,maxOpenWarnPct:100},events:[]};return res.status(200).json({...a,persistent:persistentStoreConfigured()})}
  if(req.method==='PUT'){const old=(await getJson(key))||{rules:{},events:[]};const r=req.body?.rules||{};const clamp=(v,a,b,d)=>{const n=Number(v);return Number.isFinite(n)?Math.max(a,Math.min(b,n)):d};old.rules={nearStopPct:clamp(r.nearStopPct,0.1,20,old.rules.nearStopPct||1),nearTargetPct:clamp(r.nearTargetPct,0.1,20,old.rules.nearTargetPct||1),dailyLossWarnPct:clamp(r.dailyLossWarnPct,10,100,old.rules.dailyLossWarnPct||80),maxOpenWarnPct:clamp(r.maxOpenWarnPct,10,100,old.rules.maxOpenWarnPct||100)};await setJson(key,old);return res.status(200).json({ok:true,rules:old.rules})}
