...
import argparse, json, math, shutil, subprocess, sys, time, urllib.parse, urllib.request
...
UA = "SteliosTravelVideo/1.1 (Wikimedia Commons attribution workflow)"
...
def resolve_commons(images):
    ...
    for page in payload.get("query",{}).get("pages",{}).values():
        ...
        if not title.startswith("File:") or "imageinfo" not in page: continue
...
def make_scene(src: Path, dst: Path):
...
def main():
    ...
    assert m["approved"] is True and m["publish_to_social"] is True and m["voice"]==VOICE and m["background_music"] is False and m["min_duration_seconds"]>=80
    ...
    qa={"id":"travel-monemvasia-20260924-1830","publish_to_social":True,"voice":VOICE,"duration_seconds":round(fd,3),"resolution":"1080x1920","burned_subtitles":True,"background_music":False,"avatar_presenter":False,"audio_streams":1,"photo_count":len(imgs),"all_visuals_real_photos":True,"cta_exact":True,"min_duration_gate_passed":fd>80}
    (result/"qa.json").write_text(json.dumps(qa,ensure_ascii=False,indent=2),encoding="utf-8"); (result/"script.txt").write_text("\n\n".join(seg),encoding="utf-8"); shutil.copy2(srt,result/"subtitles.srt"); (result/"visual-attribution.json").write_text(json.dumps(imgs,ensure_ascii=False,indent=2),encoding="utf-8"); print(json.dumps(qa,ensure_ascii=False,indent=2))
