...
const settings={slippageBps:0,commissionBps:0,maxHold:20,barIntervalMinutes:5};
...
let first=await updateForwardLedger(user,{fetched:base,evaluations:[evaluation],strategy,settings});assert.equal(first.createdSignals.length,1);assert.ok(first.createdSignals[0].expiresAt);
const duplicate={...evaluation,setup:{...evaluation.setup,asOf:'2026-09-25T10:05:00Z'}};const second=await updateForwardLedger(user,{fetched:base,evaluations:[duplicate],strategy,settings});assert.equal(second.createdSignals.length,0);let rows=await getSignals(user,20);let sig=rows.find(x=>x.strategyId===strategy.id&&x.symbol==='XYZ');assert.ok(sig);assert.equal(sig.direction,'SHORT');assert.equal(sig.status,'OPEN');
