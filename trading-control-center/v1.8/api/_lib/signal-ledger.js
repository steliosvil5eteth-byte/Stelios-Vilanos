...
export async function updateForwardLedger(user,{fetched=[],evaluations=[],strategy,settings={}}={}){
  ...
  for(const ev of evaluations.filter(x=>x.accepted&&x.setup)){
    ...
    rows.unshift({id,symbol:ev.symbol,strategyId:strategy.id,strategyName:strategy.name,asOf:ev.setup.asOf,createdAt:new Date().toISOString(),status:'OPEN',direction:String(ev.setup.direction||'LONG'),entry:n(ev.setup.entry),stop:n(ev.setup.stop),target:n(ev.setup.target),score:n(ev.setup.score),signalScore:n(ev.signalScore,ev.setup.score),eventScore:n(ev.event?.score),eventDirection:String(ev.event?.direction||'NEUTRAL'),headline:String(ev.event?.articles?.[0]?.title||'').slice(0,240),rr:n(ev.setup.rr),stopLossPct:n(ev.setup.stopLossPct,5),targetPct:n(ev.setup.targetPct),oos:ev.oos||null,maxHold:Math.max(1,n(settings.maxHold,20)),provider:providerBySymbol.get(ev.symbol)||'unknown',updatedAt:new Date().toISOString()});changed++;
  }
